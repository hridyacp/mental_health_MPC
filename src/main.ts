...
const depressedInput = document.getElementById('depressed-input') as HTMLInputElement;
...
const chatValueElement = document.getElementById('chat-value') as HTMLSpanElement;
...
async function handleSubmitNumber() {
  ...
  const result = await app.mpcLargest(Number(interestInput.value+depressedInput.value+sleepInput.value+energyInput.value));

  step4.classList.add('hidden');
  step5.classList.remove('hidden');
  resultValueElement.textContent = result!=0 ? "Matched !!!" : "Not Matched !!!";
